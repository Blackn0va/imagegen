...
def _test_discord() -> None:
    print("\n== Discord und Umschaltung ==")
    from tests import test_discord

    test_discord.run(check)
...
def _test_build_script() -> None:
    ...
    # Steuerzeichen in der Dokumentation.

    #

    # Beim Schreiben über Skripte wird aus "build\ffmpeg" schnell ein

    # echtes Formfeed und aus ".\build-windows" ein Backspace. Im Editor
    ...
    #
    # Sie duerfen im Quelltext stehen - aber sie muessen EINHEITLICH
    # markiert sein, damit ein Release-Check sie findet. Ein Platzhalter,
    # der nicht als solcher zu erkennen ist, geht mit in die Auslieferung.
    offen: list[str] = []
    for name in ("AGB.md", "LICENSE", "SECURITY.md"):
        datei = ROOT / name
        if not datei.is_file():
            continue
        text = datei.read_text(encoding="utf-8")
        for treffer in re.findall(r"\[[^\]\n]{2,60}\]", text):
            innen = treffer[1:-1]
            # Markdown-Links sind keine Platzhalter.
            if innen.startswith("http") or innen.endswith(".md"):
                continue
            if innen.isupper() or "eintragen" in innen.lower():
                offen.append(f"{name}: {treffer}")

    check(
        "offene Platzhalter sind als solche erkennbar",
        all("eintragen" in eintrag.lower() for eintrag in offen),
        "; ".join(e for e in offen if "eintragen" not in e.lower()),
    )
    if offen:
        print(f"  Hinweis  {len(offen)} Platzhalter offen (vor Auslieferung ausfüllen):")
...
# Steuerzeichen, die in einer Textdatei nichts zu suchen haben.
# Als Codepunkte geschrieben, damit sie beim Bearbeiten dieser
# Datei nicht selbst zu echten Steuerzeichen werden.
CONTROL_CHARS = "".join(chr(n) for n in (12, 8, 11, 7, 0))
